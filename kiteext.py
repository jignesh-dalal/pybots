...
class KiteExt(KiteConnect):
    ...
    def login_with_credentials(self, userid, password, twofa):
        ...
        r2 = self.reqsession.post(self.root + self._routes['api.twofa'], data={
        ...
        login_cookies = [{'domain': "kite.zerodha.com",'name': c.name, 'path': c.path, 'secure': True, 'value': c.value} for c in r2.cookies]
        # print(str(json.dumps(login_cookies)))
        # print("\n\n\n")
        
        cookie_data = r2.cookies.get_dict()
        cookies_array = [{"name": c.name, "value": c.value,"domain": c.domain, "path": c.path,"expires/max-age": "Session",} for c in r2.cookies]
        cookies_array = json.dumps(cookies_array, indent=4, ensure_ascii=False)
        # print(cookies_array)
        # print("\n\n\n")
        self.enctoken = r2.cookies.get('enctoken')
        self.public_token = r2.cookies.get('public_token')
        self.user_id = r2.cookies.get('user_id')

        self.headers['Authorization'] = 'enctoken {}'.format(self.enctoken)
        return self.enctoken, self.public_token
    ...
    def kws(self, api_key='kitefront'):
        return KiteExtTicker(api_key=api_key, access_token='&user_id='+self.user_id+'&enctoken='+urllib.parse.quote(self.enctoken), root='wss://ws.zerodha.com')
    ...
    def ticker(self, api_key='kitefront', enctoken=None, userid=None):
        ...
        return KiteExtTicker(api_key=api_key, access_token='&user_id='+self.user_id+'&enctoken='+urllib.parse.quote(self.enctoken), root='wss://ws.zerodha.com',reconnect_max_tries=300)
